...
class UserLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        if user:
            ...
            if session_key:
                # Удаляем старые авторизованные пользовательские корзины
                forgot_carts = Cart.objects.filter(user=user)
                if forgot_carts.exists():
                    forgot_carts.delete()
                # Добавить нового авторизованного пользователя из сеанса корзины
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "Дубок - Авторизация"
        return context
...
class UserRegistrationView(CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"{user.username}, Вы зарегистрировались и вошли в аккаунт!"
        )
        return HttpResponseRedirect(self.get_success_url())

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = "Дубок - Регистрация"
        return context
...
class UserProfileView(LoginRequiredMixin, CacheMixin,UpdateView):
    ...
    def get_object(self, queeryset=None):
...
#             if sessions_key:
#                 Cart.objects.filter(session_key=sessions_key).update(user=user)
